...
st.set_page_config(
    page_title="Mestre dos Greens PRO - V66.4 (Fix)",
    page_icon="⚽",
    layout="wide",
...
if not df_recent.empty:
    ...
    # 2. WINRATE (ATUALIZADO V66.4 - DETALHADO)
        ...
        def calculate_winrate(target_date_start, target_date_end):
            mask = (df_recent['Date'] >= target_date_start) & (df_recent['Date'] <= target_date_end)
            games = df_recent.loc[mask]
            
            if games.empty:
                st.warning("Sem jogos finalizados neste período.")
                return
            ...
            for i, row in games.iterrows():
                ...
                if probs['Over15'] >= 0.80:
                    ...
                    total_bets_geral += 1
                    is_green = (row['FTHG'] + row['FTAG']) > 1.5
                    if is_green:
            ...
            if total_bets_geral > 0:
                ...
                for market, data in market_stats.items():
                    with cols[idx]:
                        t = data['total']; g = data['green']
                        wr = (g/t*100) if t>0 else 0
                        color = "#2ea043" if wr >= 70 else "#f1c40f" if wr >= 50 else "#da3633"
                        st.markdown(f"""<div class="metric-card"><div style="color:#8b949e">{market}</div><div style="font-size:22px;font-weight:bold;color:{color}">{wr:.1f}%</div><div style="font-size:12px">{g}/{t}</div></div>""", unsafe_allow_html=True)
                    idx+=1
        ...
        with tab_yesterday:
            ...
            st.subheader(f"Ontem ({yesterday.strftime('%d/%m')})")
            if st.button("Calcular Ontem"):
                calculate_winrate(yesterday, yesterday + pd.Timedelta(hours=23, minutes=59))
                
        with tab_month:
            first_day = pd.Timestamp.now().replace(day=1).normalize()
            st.subheader(f"Mês ({first_day.strftime('%B')})")
            if st.button("Calcular Mês"):
                calculate_winrate(first_day, pd.Timestamp.now())
